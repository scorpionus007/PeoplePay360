...
async function list(req, res) {
  ...
  const rows = await models.MobilityPartner.findAll({ where, order: [['name', 'ASC']] });
  return success(res, rows);
}

async function getOne(req, res) {
  const row = await models.MobilityPartner.findOne({
...
async function create(req, res) {
  const row = await models.MobilityPartner.create({
    ...req.body,
